...
int main(int argc, char** argv)
{
    ...
    std::sort(filtered.begin(), filtered.end(), [](const Record& a, const Record& b) {
        ...
        std::string ap = a.values.count("chemId") && !a.values.find("chemId")->second.empty() ? a.values.find("chemId")->second[0] : "";
        std::string bp = b.values.count("chemId") && !b.values.find("chemId")->second.empty() ? b.values.find("chemId")->second[0] : "";
        return ap < bp;
    });
