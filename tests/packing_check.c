...
int main(int argc, char* argv[]) {
  ...
  while (grib_read_any_from_file(c,fin,buffer,&length)==GRIB_SUCCESS) {
	  ...
	  for (i=0;i<nvalues;i++) {
		  if (gvalues[i] != values[i]) {
			printf("ERROR gribex[%ld]=%.20e grib_api[%ld]=%.20e \n",i,gvalues[i],i,values[i]);
			exit(1);
		  }
	  }
