...
int main(int argc, char* argv[]) {
  ...
  char* finname;
  char* ofilename;
  char defofilename[]="gribex_perf.out";
  double *values=NULL;
  ...
  int repeatso=1;
  int repeatsimple=1;
  grib_timer *tega,*tdga,*tegx,*tdgx;
  ...
  tega=grib_get_timer(0,"encoding grib_api", 0, 0);
  tdga=grib_get_timer(0,"decoding grib_api", 0, 0);
  tegx=grib_get_timer(0,"encoding gribex", 0, 0);
  tdgx=grib_get_timer(0,"decoding gribex", 0, 0);
  ...
  *outfilename='\0';
  ...
  grib_timer_start(tegx);
  gribex_check(cgribex(  miss, ksec0,ksec1,ksec2,rsec2,ksec3,rsec3,
                      ksec4,gvalues,nvalues, buf,&gribex_msg_len,"C"));
  grib_timer_stop(tegx,0);
  ...
  *outfilename='\0';
  ...
  print_timer(tegx,repeatso);
  print_timer(tdgx,repeatso);
  print_timer(tega,repeatsimple);
  print_timer(tdga,repeatsimple);

  fprintf(fout,"%s %s %ld %ld %s %g %g %g %g\n",
	shortName,gridType,(long)nvalues,bitsPerValue,
  	grid,tegx->timer_/repeatso,tega->timer_/repeatsimple,tdgx->timer_/repeatso,tdga->timer_/repeatsimple);
