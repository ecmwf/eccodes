...
void test_grib2_surface_type_requires_value()
{
    printf("Running %s ...\n", __func__);
    int err = 0;
    ECCODES_ASSERT( codes_grib2_surface_type_requires_value(103, &err) == 1 && !err );
    ECCODES_ASSERT( codes_grib2_surface_type_requires_value(160, &err) == 1 && !err );
    ECCODES_ASSERT( codes_grib2_surface_type_requires_value(10,  &err) == 0 && !err );

    ECCODES_ASSERT( codes_grib2_surface_type_requires_value(-1, &err) == 0 && err );
    ECCODES_ASSERT( codes_grib2_surface_type_requires_value(10000, &err) == 0 && err );
}
...
int main(int argc, char** argv)
{
    ...
    test_grib2_surface_type_requires_value();
