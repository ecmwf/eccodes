...
static void test_string_splitting()
{
    ...
    if (list.empty()) { ECCODES_ASSERT(!"List is empty"); return; }
    for (i = 0; i < list.size(); ++i) {} /* count how many tokens */
    ECCODES_ASSERT(i == 4);
    ...
    for (i = 0; i < list.size(); ++i) free(list[i]);

    strcpy(input, "12345|a gap|");
    list = string_split(input, "|");
    if (list.empty()) { ECCODES_ASSERT(0); return; }
    for (i = 0; i < list.size(); ++i) {} /* count how many tokens */
    ECCODES_ASSERT(i == 2);
