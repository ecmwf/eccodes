...
int main(int argc, char** argv)
{
    ...
    CODES_CHECK(codes_get_long(h, "section7Length", &sec7Len), 0);
    printf("1. codes_get_length of sec7 = %zu\n", sec7Len);

    blen     = sec7Len;
    byte_val = (unsigned char*)malloc(blen * sizeof(unsigned char));
    GRIB_CHECK(codes_get_bytes(h, "codedValues", byte_val, &blen), 0);
    ...
    fclose(f);
    len = fsize;
    // GRIB_CHECK( codes_set_bytes(h, "codedValues", p, &len), 0);
    // CODES_CHECK(codes_write_message(h, "recoded1.grib", "w"), 0);
    // codes_handle* h1 = codes_grib_handle_new_from_samples(0, "GRIB2");
    // assert(h1);
    ...
    // Normally one cannot set the referenceValue key as it is READ ONLY
    // GRIB_CHECK( grib_set_double(h1, "referenceValue", referenceValue), 0);
    ...
    GRIB_CHECK(codes_set_bytes(h1, "codedValues", p, &len), 0);
    ...
    // size_t len = blen;
    // byte_val[2]=0; // test for simple packing only
    // byte_val[3]=0;
    // GRIB_CHECK( codes_set_bytes(h, "codedValues", byte_val, &len), 0);
    // CODES_CHECK(codes_write_message(h, "recoded2.grib", "w"), 0);
