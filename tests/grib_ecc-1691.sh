...
# Setup Destine pseudocentre GRIB message
...
# Now check setting dataset to climate-dt (1). Check keys are present and equal defaults
...
# Now check an example where a few additional things are set in climate-dt
...
# Now check that mars.date and mars.time are equal to validityDate and validityTime, and that mars.step has been unaliased

[ $( ${tools_dir}/grib_get -p mars.date,mars.time ${temp_grib_a} ) = $( ${tools_dir}/grib_get -p validityDate,validityTime ${temp_grib_a} ) ]

[ $( ${tools_dir}/grib_get -f -p mars.step ${temp_grib_a} ) = "not_found" ]

# Now check setting dataset to extremes-dt (2). This time we will check string also.
...
# Finally check concepts. First check default value and then example of mapping
