...
int main(int argc, char* argv[]) {
  ...
  char* finname;
  char* ofilename;
  char defofilename[]="png_perf.out";
  double *values=NULL;
  int append=0;
  size_t nvalues=0;
  int count,e=0;
  ...
  char grid_png[]="grid_png";
  size_t grid_png_l=strlen(grid_png);
  ...
  tej=grib_get_timer(0,"encoding png", 0, 0);
  tdj=grib_get_timer(0,"decoding png", 0, 0);
  ...
  if (!append) 
    fprintf(fout,
	"shortName gridType numberOfValues bitsPerValue grid sizeSimple sizepng encodepng encodeSimple decodepng decodeSimple\n");
