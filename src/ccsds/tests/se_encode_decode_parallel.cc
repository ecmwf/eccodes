...
void testRefEncodeDecode(Span<ValueType> block)
{
    ...
    {
        // SecondExtensionEncoder<ValueType, BPV, ReferenceType::Ref, CodeOptionSet::Basic, blockSize> se{};
        // se.encode(bitBuffer);

        encoder::EncoderPar<ValueType, ValueType> encoder{BPV, blockSize, rsiSize, CodeOptionSet::Basic};
        auto seIres = encoder.sePrepare<ReferenceType::Ref>(block);
        seIres.offset = 0;
    ...
    {
        DecoderSeq<ValueType, ValueType> se{BPV, blockSize, rsiSize, CodeOptionSet::Basic};
        se.seDecode(bitBuffer, Span(decodedBlock));
    }
