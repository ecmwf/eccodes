...
static int apply_operators(grib_accessor* a)
{
    ...
    const char* firstOrderStatisticsAbbreviation           = "firstOrderStatistics";
    const char* differenceStatisticsAbbreviation           = "differenceStatistics";
    const char* associatedFieldAbbreviation                = "associatedField";
    const char* substitutedValueAbbreviation               = "substitutedValue";
    const char* associatedFieldName                        = "associated field";
    const char* associatedFieldType                        = "long";
    const char* associatedFieldUnit                        = "CODE TABLE 31021";
    ...
    for (i = 0; i < numberOfDescriptors; i++) {
        ...
        if (F == 2) {
            ...
            if (localDescriptorWidth) {
                if (scale[ielement] == GRIB_MISSING_LONG)
                    scale[ielement] = 0;
                if (reference[ielement] == GRIB_MISSING_LONG)
                    reference[ielement] = 0;
                if (width[ielement] == GRIB_MISSING_LONG)
                    width[ielement] = localDescriptorWidth;
            ...
            if (associatedFieldWidth && X != 31) {
                ...
                for (id = 0; id < delayedReplicationCount; id++)
                    delayedReplication[id]--;
                j++;
            }
            ...
            /* bitmap */
            if (X == 31 && Y == 31) {
                self->bitmapNumber[j] = bitmapNumber;
            }
...
static int unpack_long(grib_accessor* a, long* val, size_t* len)
{
    ...
    *len = self->expandedAOSize;
    switch (self->index) {
        case 0:
            for (i = 0; i < *len; i++)
                val[i] = self->expandedAO[i];
            break;
        case 1:
            for (i = 0; i < *len; i++)
                val[i] = self->scaleAO[i];
            break;
        case 2:
            for (i = 0; i < *len; i++)
                val[i] = self->referenceAO[i];
            break;
        case 3:
            for (i = 0; i < *len; i++)
                val[i] = self->widthAO[i];
            break;
        case 4:
            for (i = 0; i < *len; i++)
                val[i] = self->bitmapNumber[i];
            break;
        case 5:
            for (i = 0; i < *len; i++)
                val[i] = self->associatedBitmapNumber[i];
            break;
        case 6:
            for (i = 0; i < *len; i++)
                val[i] = self->associatedBitmapIndex[i];
            break;
        case 11:
            for (i = 0; i < *len; i++)
                val[i] = self->associatedInfoNumber[i];
            break;
        default:
            Assert(0);
    }
