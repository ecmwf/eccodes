...
def create_cpp_container_buffer_arg(name):
...
def create_cpp_container_length_arg(name):
    cpp_member = create_cpp_container_buffer_arg(name)
...
def cbuffer_to_buffer_mapping(cbuffer, conversion_data):
...
def clength_to_buffer_mapping(clength, conversion_data):
...
def cname_to_cpp_container_buffer(cname, conversion_data):

    # 1. Check buffer maps
    buf_map = cbuffer_to_buffer_mapping(cname, conversion_data)
    if buf_map:
        return create_cpp_container_buffer_arg(buf_map.cpp_container.name)
    ...
    if cpparg and conversion_data.is_container_type(cpparg.decl_spec.type):
        return create_cpp_container_buffer_arg(cpparg.name)

    return None

# Tries to determine if the supplied cname maps to a C++ container type.
# If a match is found, returns a StructMemberAccess representing "cpp_container.size()", else None
def cname_to_cpp_container_length(cname, conversion_data):

    # 1. Check buffer maps
    buf_map = clength_to_buffer_mapping(cname, conversion_data)
    if buf_map:
        return create_cpp_container_length_arg(buf_map.cpp_container.name)

    # 2. Try standard arg conversions
    cpparg = conversion_data.funcbody_cpparg_for_carg_name(cname)
    if cpparg and conversion_data.is_container_type(cpparg.decl_spec.type):
        return create_cpp_container_length_arg(cpparg.name)
...
def cname_to_cpp_container(cname, conversion_data):

    buf_map = clength_to_buffer_mapping(cname, conversion_data)
    if buf_map:
        return create_cpp_container_length_arg(buf_map.cpp_container.name)
    
    return cname_to_cpp_container_buffer(cname, conversion_data)