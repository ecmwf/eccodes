...
void grib_accessor_gen_t::init(const long len, grib_arguments *param) {
  ...
  if (flags_ & GRIB_ACCESSOR_FLAG_TRANSIENT) {
    length_ = 0;
    if (!vvalue_)
      vvalue_ = (grib_virtual_value *)grib_context_malloc_clear(
          context_, sizeof(grib_virtual_value));
    ...
    if (act->default_value != NULL) {
      ...
      double d;
      char tmp[1024];
      grib_expression *expression = grib_arguments_get_expression(
          grib_handle_of_accessor(this), act->default_value, 0);
      int type = grib_expression_native_type(grib_handle_of_accessor(this),
                                             expression);
      switch (type) {
        // TODO(maee): add single-precision case

      case GRIB_TYPE_DOUBLE:
        grib_expression_evaluate_double(
            grib_handle_of_accessor(this), expression, &d);
        pack_double(&d, &s_len);
        break;

      case GRIB_TYPE_LONG:
        grib_expression_evaluate_long(
            grib_handle_of_accessor(this), expression, &l);
        pack_long(&l, &s_len);
        break;

      default:
        s_len = sizeof(tmp);
        p = grib_expression_evaluate_string(
            grib_handle_of_accessor(this), expression, tmp, &s_len, &ret);
        if (ret != GRIB_SUCCESS) {
          grib_context_log(context_,
                           GRIB_LOG_ERROR,
                           "Unable to evaluate %s as string",
                           name_);
          Assert(0);
        }
        s_len = strlen(p) + 1;
...
long grib_accessor_gen_t::next_offset() { return offset_ + length_; }
...
size_t grib_accessor_gen_t::string_length() { return 1024; }

long grib_accessor_gen_t::byte_count() { return length_; }
...
long grib_accessor_gen_t::byte_offset() { return offset_; }
...
int grib_accessor_gen_t::pack_expression(grib_expression *e) {
  ...
  case GRIB_TYPE_LONG: {
    ...
    if (ret != GRIB_SUCCESS) {
      grib_context_log(context_,
                       GRIB_LOG_ERROR,
                       "Unable to set %s as long (from %s)",
                       name_,
                       e->cclass->name);
      return ret;
    }
  ...
  case GRIB_TYPE_DOUBLE: {
    ...
    if (ret != GRIB_SUCCESS) {
      grib_context_log(context_,
                       GRIB_LOG_ERROR,
                       "Unable to set %s as double (from %s)",
                       name_,
                       e->cclass->name);
      return ret;
    }
  ...
  case GRIB_TYPE_STRING: {
    ...
    if (ret != GRIB_SUCCESS) {
      grib_context_log(context_,
                       GRIB_LOG_ERROR,
                       "Unable to set %s as string (from %s)",
                       name_,
                       e->cclass->name);
      return ret;
    }
...
grib_section *grib_accessor_gen_t::sub_section() { return NULL; }
...
grib_accessor *grib_accessor_gen_t::next_accessor() { return next(this, 1); }
...
size_t grib_accessor_gen_t::preferred_size(int from_handle) { return length_; }

int grib_accessor_gen_t::is_missing_internal() { return is_missing(); }
...
long grib_accessor_gen_t::get_next_position_offset() { return next_offset(); }

grib_accessor_gen_t::~grib_accessor_gen_t() {}

void grib_accessor_gen_t::post_init() { return; }
