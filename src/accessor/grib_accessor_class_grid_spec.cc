...
int grib_accessor_grid_spec_t::unpack_string(char* val, size_t* len)
{
#if defined(HAVE_GEOGRAPHY) && defined(HAVE_ECKIT_GEO)
    ECCODES_ASSERT(0 < *len);
    ECCODES_ASSERT(val != nullptr);
    ...
    std::strncpy(val, spec_str.c_str(), *len);
    ECCODES_ASSERT(val[length] == '\0');
