...
class DataG22orderPackingDataFuncSigConverter(private_method_funcsig_conv.PrivateMethodFuncSigConverter):
    func_conversions = [
        ...
        FuncSigMapping(FuncSig("int", "unpack_helper", [Arg("grib_accessor*", "a"), Arg("T*", "val"), Arg("const size_t*", "len")]),
                       FuncSig("GribStatus", "unpackHelper", [None, Arg("const std::vector<T>&", "vecTValues"),  None]),
...
class DataG22orderPackingDataAccessorSpecific(AccessorSpecific):
    def __init__(self) -> None:
        super().__init__()

    def update_converters(self, converters):
        converters[Converter.PRIVATE_METHOD_FUNCSIG] = DataG22orderPackingDataFuncSigConverter
        return converters
