...
'''class Member:
    def __init__(self, s) -> None:
        ...
        self.name = arg.transform_variable_name(self.cname) + "_"
...
class Function:
    def __init__(self, name, return_type, args, template=None) -> None:
        ...
        if isinstance(args, str):
            for entry in [a.strip() for a in args.split(",")]:
                if not entry:
                    continue
                carg = arg.Arg.from_string(entry)
...
class FunctionDelegate:
    ...
    def transform_args(self):
        arg_map = {}
        for entry in self._cargs:
            arg_map[entry] = to_cpp_func_sig_arg(entry)

        return arg_map

    @property
    def transformed_name(self):
        return arg.transform_function_name(self._name)

    @property
    def transformed_args(self):
        args = []
        for entry in self._cargs:
            if entry in self._arg_map:
                cpp_arg = self._arg_map[entry]
                if cpp_arg:
                    args.append(cpp_arg)
    ...
    def process_type_declarations(self, line):
        ...
        if m:
            ctype = m.group(1)
            cpptype = arg.transform_class_name(ctype)
            ...
            cpp_arg = arg.Arg("struct", cpptype)
    ...
    def process_function_pointers(self, line):
        ...
        if m:
            ...
            for arg_type in [a.strip() for a in m.group(3).split(",")]:
                cpp_sig_arg = to_cpp_func_sig_arg(arg.Arg(arg_type, "Dummy"))
                if cpp_sig_arg:
    ...
    def convert_grib_utils(self, line):
        for util in convert_data.GribUtilFuncs:
            ...
            if m:
                cpp_util = arg.transform_function_name(util)
    ...
    def apply_function_transforms(self, line):
        ...
        for f in self._owner_class._static_functions:
            ...
            if m:
                ...
                line = re.sub(m.re, rf"{prefix}{arg.transform_function_name(f.name)}", line)
    ...
    def update_line(self, line):
        ...
        for update_func in update_functions:
            ...
            #debug_line("update_line", f"[Before] {update_func.__name__}")
            line = update_func(line)
            #debug_line("update_line", f"[After ] {update_func.__name__}")
...
class ConstructorMethod(Method):
    ...
    def transform_args(self):
        arg_map = {}
        for entry in self._cargs:
            if entry.type == "grib_arguments*":
                arg_map[entry] = arg.Arg("AccessorInitData const&", "initData")
            else:
                arg_map[entry] = None
...
class CompareMethod(Method):
    ...
    def transform_args(self):
        ...
        return { 
            cargs[0] : None,
            cargs[1] : arg.Arg("AccessorData const&", "rhs")
        }
...
class Class:
    ...
    def finalise(self, other_classes):
        ...
        for name, f in list(self._functions.items()):
            ...
            for entry in f.args:
                if entry.type in ptr_type_names:
    ...
    def name_camel_case(self):
        return arg.transform_variable_name(self._name)
    ...
    def tidy_class_name(self, path):
        ...
        name = arg.transform_class_name(name)
    ...
    def update_class_members(self, line):
        ...
        accessor_variable_name = arg.transform_variable_name(self._name)
        ...
        for m in self.members_in_hierarchy:
            ...
            line,count = re.subn(rf"\b{accessor_variable_name}->{m.name}\b", rf"{m.name}", line)
            if(count):
    ...
    def update_static_function_calls(self, line):
        for m in self._static_functions:
            name = m.name
            line,count = re.subn(rf"\b{name}\s*\(", f"{arg.transform_function_name(name)}(", line)
            if(count):
    ...
    def tidy_top_level(self, line):
        for m in self._static_functions:
            name = m.name
            line,count = re.subn(rf"\b{name}\b", f"{arg.transform_function_name(name)}", line)
...
def parse_file(path):
    ...
    if definitions:
        ...
        accessorImpl = Accessor(
            ...
            members=[arg.Member.from_string(m) for m in definitions.get("MEMBERS", []) if m != ""],
        )
        return accessorImpl
