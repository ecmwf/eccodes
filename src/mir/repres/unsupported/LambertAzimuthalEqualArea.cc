...
LambertAzimuthalEqualArea::LambertAzimuthalEqualArea(const param::MIRParametrisation& parametrisation)
    ...
    firstXY_ = Point2(kp * cos_phi * sin_dlambda,
                      kp * (cos_phi1 * sin_phi - sin_phi1 * cos_phi * cos_dlambda));
    ...
    auto f = [=](double x, double y) -> Point2 {
        ...
        return Point2{
            lambda0 + std::atan2(x * sin_c, rho * cos_phi1 * cos_c - y * sin_phi1 * sin_c),
    ...
    Point2 corners[] = { f(min[0], max[1]), f(max[0], max[1]),
                         f(min[0], min[1]), f(max[0], min[1]) };
...
Iterator* LambertAzimuthalEqualArea::iterator() const {
        ...
        bool next(Latitude& _lat, Longitude& _lon) {
            ...
            if (j_ < ny_ && i_ < nx_) {
                ...
                if (eckit::types::is_approximately_equal(rho, 0.)) {
                    ...
                    _lon = lon(util::radian_to_degree(lambda0_ + std::atan2(x_ * sin_c, rho * cos_phi1_ * cos_c - y_ * sin_phi1_ * sin_c)));
                }
    ...
    public:
        LAEAIterator(double radius,
                     const PointLatLon& reference,
                     const Point2& firstXY,
                     size_t nx,
                     size_t ny,
                     double Dx,
                     double Dy)
...
bool LambertAzimuthalEqualArea::sameAs(const Representation& other) const {
    ...
    return o
            && nx_ == o->nx_
            && ny_ == o->ny_
            && eckit::types::is_approximately_equal(radius_, o->radius_)
            && eckit::types::is_approximately_equal(Dx_, o->Dx_)
            && eckit::types::is_approximately_equal(Dy_, o->Dy_)
            && reference_.lon() == o->reference_.lon()
            && reference_.lat() == o->reference_.lat()
            && firstLL_.lon() == o->firstLL_.lon()
            && firstLL_.lat() == o->firstLL_.lat();
}
