...
int Latlon::next(double* lat, double* lon, double* val) const
{
    ...
    if (!jPointsAreConsecutive_) {
        ...
        ret_lat = lats_[static_cast<long>(floor(e_ / Ni_))];
        ret_lon = lons_[e_ % Ni_];
        if (data_) {
            ret_val = data_[e_];
        }
    }
        ...
        ret_lon = lons_[e_ / Nj_];
        ret_lat = lats_[static_cast<long>(floor(e_ % Nj_))];
        if (data_) {
            ret_val = data_[e_];
        }
    }
...
int Latlon::init(grib_handle* h, grib_arguments* args)
{
    ...
    if ((grib_is_missing(h, s_jdir, &err) && err == GRIB_SUCCESS) || (jdir == GRIB_MISSING_DOUBLE)) {
        ECCODES_ASSERT(Nj_ > 1);
        if (lat1 > lat2) {
            jdir = (lat1 - lat2) / static_cast<double>(Nj_ - 1);
        }
        else {
            jdir = (lat2 - lat1) / static_cast<double>(Nj_ - 1);
        }
