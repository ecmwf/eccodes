...
static int value_count(grib_accessor* a, long* number_of_points)
{
    ...
    *number_of_points = 0;
    if ((ret = grib_get_long(grib_handle_of_accessor(a), self->number_of_points, number_of_points)) != GRIB_SUCCESS) {
...
static int unpack_double(grib_accessor* a, double* val, size_t* len)
{
    ...
    if ((err = grib_get_double_array_internal(grib_handle_of_accessor(a), self->coded_values, coded_vals, &size)) != GRIB_SUCCESS) {
