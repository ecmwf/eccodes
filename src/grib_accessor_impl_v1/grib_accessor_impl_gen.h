...
namespace eccodes {
    ...
    public:
        ...
        int unpack_bytes(unsigned char* val, size_t* len) final;
        int unpack_double(double* val, size_t* len) final;
        int unpack_float(float* val, size_t* len) final;
    ...
    protected:
        virtual int pack_bytes(pack_buffer& bytes, std::size_t* packed_len);
        ...
        virtual int unpack_bytes(byte_view bytes);
    ...
    private:
        void init_gen(const long len, grib_arguments* params);
    };
}
